...
在开始训练之前，请确保你的 Windows 设备上已安装最新版的 WSL（Windows Subsystem for Linux） 2 。 
...
![runner-lora-open-WSL](./runner-lora-open-WSL.png)
...
![alt text](./runner-lora-install-ubuntu.png)
...
![runner-lora-install-ubuntu2](./runner-lora-install-ubuntu2.png)
...
![runner-lora-install-ev](./runner-lora-ev.webp)
...
![runner-lora-model](./runner-lora-model.webp)
...
- 数据路径：这里的数据指的用于 LoRA 微调的数据集，这些数据集用于训练 RWKV 模型理解和学习特定任务，例如情感分析、文本分类、问答系统等。数据路径必须是一个**文件夹**或者具体的**jsonl 格式文件** (jsonl 数据集的格式可参考：[RWKV微调数据集](https://rwkv.cn/RWKV-Fine-Tuning/FT-Dataset)，未来会支持更多格式)。当你填写的路径是一个文件夹时，该文件夹内的所有 txt 文件会被自动转换为训练数据，通常用于大批量训练写作，代码生成或知识库。
...
![runner-lora-example](./runner-lora-example.png)
---
![runner-lora-example1](./runner-lora-example1.png)

此次 LoRA 微调使用全新的 180M 全中文数据集：

![runner-lora-example-data](./runner-lora-example-data.png)

使用如下参数，在 RWKV-Runner 中进行 21 个epoch 的 LoRA 微调：

![runner-lora-example](./runner-lora-example-parameters.png)
