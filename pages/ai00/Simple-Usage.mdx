...
keywords: ai00 rwkv server,ai00_rwkv_server,ai00下载,Ai00安装
---
...
接着将 `.pth` 文件移动到 `dist/assets` 目录中，启动终端并执行以下命令，将指定的 `.pth` 模型转化成 `.st` 模型：
```bash copy
...
![ai00-transform-example](./imgs/ai00-transform-example.png) 
...
![ai00-create-model-dir](./imgs/ai00-create-model-dir.png) 
...
![Ai00-homepage](./imgs/Ai00-homepage.png)
...
<Callout type="info" emoji="ℹ️">
更详细的解码参数解释和参数推荐查看[RWKV 的解码参数](https://rwkv.cn/RWKV-Prompts/RWKV-Parameters)
</Callout>
