...
为了突出 state tuning 的特性，我们选取了大量 emoji 数据作为训练数据：

![state-tuning-dataset-demo](./imgs/state-tuning-dataset-demo.png)

### 转化成 binidx 数据

使用 [`make_data.py`](https://github.com/BlinkDL/RWKV-LM/blob/main/RWKV-v5/make_data.py) 脚本，将 jsonl 训练数据乱序重复生成 10 遍，同时转化成可供 RWKV 训练的 binidx 数据。

在 Linux 工作区依次运行以下命令，以使用 `RWKV-LM` 仓库中的 `make_data.py` 脚本生成 binidx 数据：

``` bash copy
clone https://github.com/BlinkDL/RWKV-LM.git
cd RWKV-LM/RWKV-v5
python make_data.py /home/rwkv/RWKV-PEFT/data/qaemoji.jsonl 10 512
```
