...
在 Linux 或 Mac 系统上，可以使用命令行工具进行数据重复和打乱：

``` bash copy
# 将 data.jsonl 文件重复三次，并输出到 repeated_data.jsonl 文件中
cat data.jsonl data.jsonl data.jsonl > repeated_data.jsonl
# 将 repeated_data.jsonl 文件的行打乱，并将打乱的结果输出到 shuffled_data.jsonl 文件中
shuf repeated_data.jsonl > shuffled_data.jsonl
```
