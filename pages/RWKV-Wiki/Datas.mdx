...
import { LineChart } from '/components/docs/line-chart.tsx'
...
<Tabs items={[' 热力图 ', ' 雷达图 ', ' 原始数据 ']}>
 ...
 为了更直观地观察模型在每项评测上的表现，我们对原始数据进行了归一化处理。
      ...
      <RadarChartComponent chartDataIndex={0}/> 
  </Tabs.Tab>
  <Tabs.Tab> 

| Name                                    | Average (lower=better) | ao3 english | bbc news | wikipedia english | arxiv computer science | arxiv physics | github cpp | github python |
|-----------------------------------------|------------------------|-------------|----------|-------------------|------------------------|---------------|------------|---------------|
| Meta-Llama-3.1-8B                      | 7.227                  | 10.529      | 8.201    | 7.932             | 7.549                  | 7.686         | 4.204      | 4.487         |
| Qwen2.5-7B                             | 7.453                  | 10.931      | 8.742    | 8.815             | 7.683                  | 7.991         | 3.920      | 4.091         |
| Qwen2-7B                               | 7.528                  | 10.814      | 8.582    | 8.709             | 7.822                  | 8.249         | 4.204      | 4.317         |
| Mistral-7B-v0.1                        | 7.580                  | 10.546      | 8.282    | 8.435             | 7.852                  | 8.092         | 4.800      | 5.051         |
| RWKV-6-World-7B-v2.1 | 7.817                  | 10.410      | 8.740    | 8.577             | 8.107                  | 8.248         | 5.122      | 5.516         |
| RWKV-5-World-7B-v2   | 7.905                  | 10.495      | 8.878    | 8.694             | 8.184                  | 8.311         | 5.187      | 5.587         |
| Qwen1.5-7B                             | 7.916                  | 11.100      | 9.126    | 9.357             | 7.953                  | 8.107         | 4.845      | 4.924         |
| Zamba-7B-v1                            | 8.089                  | 10.846      | 8.520    | 8.639             | 8.060                  | 8.290         | 5.938      | 6.328         |
...
<Tabs items={[' 热力图 ', ' 雷达图 ', ' 原始数据 ']}>
 ...
 为了更直观地观察模型在每项评测上的表现，我们抽取了平均分排行前七的模型，并对原始数据进行了归一化处理。
      ...
      <RadarChartComponent chartDataIndex={1}/> 
  </Tabs.Tab>
  <Tabs.Tab> 
...
<Tabs items={[' 热力图 ', ' 雷达图 ', ' 原始数据 ']}>
 ...
 为了更直观地观察模型在每项评测上的表现，我们抽取了平均分排行前七的模型，并对原始数据进行了归一化处理。
      ...
      <RadarChartComponent chartDataIndex={10}/> 
  </Tabs.Tab>
  <Tabs.Tab> 
...
<Tabs items={[' 热力图 ', ' 雷达图 ', ' 原始数据 ']}>
 ...
 为了更直观地观察模型在每项评测上的表现，我们抽取了平均分排行前七的模型，并对原始数据进行了归一化处理。
      ...
      <RadarChartComponent chartDataIndex={11}/> 
  </Tabs.Tab>
  <Tabs.Tab> 
...
| **RWKV-6-1B6-v2.1**| 1.600       | 8.676                  | 11.240       | 9.546    | 9.595              | 8.842                    | 9.145          | 6.002       | 6.364          |
...
RWKV-6-World-7B-v2.1 模型在 RULER 测试的各项得分如下表：
...
以下是 RWKV-6-World-7B-v2.1 和其他模型的 LongBench 测试分数对比，数据表格按照六个分类展示：
