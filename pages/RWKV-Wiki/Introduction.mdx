...
有关 RWKV5/6 论文的解读，可以参考[此文章](https://rwkv.cn/news/read?id=382345)。
</Callout>

RWKV-7 架构是 RWKV 的最新版本，架构的训练和推理代码均已定稿，架构论文正在由彭博和社区成员联合撰写。

RWKV-7 采用了动态状态演化（Dynamic State Evolution），超越了 attention / linear attention 范式 $TC$0 表达能力的根本限制。
<Tabs items={['RWKV-7 架构图', 'RWKV-6 架构图']}>
  <Tabs.Tab>
RWKV-7 架构图如下： 
...
从 RWKV-4 架构开始，RWKV 在每个版本都发布了多种参数规模的开源模型。

截至 2024 年 12 月，RWKV-4、RWKV-5 模型的生命周期已结束。推荐使用最新版本的 RWKV-6-world-V3 模型。得益于最新的架构设计，RWKV-6 模型在所有任务上的表现都更好。

| 版本 | RWKV-V4 | RWKV-v5-Eagle | RWKV-v6-Finch | RWKV-v7-Goose |
| --- | --- | --- | --- | --- |
| 论文 | 🎓[论文已发布](https://arxiv.org/abs/2305.13048) | 🎓[论文已发布](https://arxiv.org/abs/2404.05892) | 🎓[论文已发布](https://arxiv.org/abs/2404.05892) | **✍️ 撰写中** |
| 总体状态 | **⚠ EOL** | **⚠ EOL**| **✨ 稳定版本** | **⚙️ 模型训练中** |
| 0.4B 模型 | ✅ 已发布 | ✅ 已发布 | ❌ 无计划 |✅ [已发布](https://hf-mirror.com/BlinkDL/rwkv-7-world/blob/main/RWKV-x070-World-0.4B-v2.9-20250107-ctx4096.pth) |
| 1.5B 模型 | ✅ 已发布 | ✅ 已发布| ✅ [已发布](https://hf-mirror.com/BlinkDL/rwkv-6-world/blob/main/RWKV-x060-World-1B6-v2.1-20240328-ctx4096.pth) |**⚙️ 2025年1月下旬** |
| 3B 模型 | ✅ 已发布 | ✅ 已发布 | ✅ [已发布](https://hf-mirror.com/BlinkDL/rwkv-6-world/blob/main/RWKV-x060-World-1B6-v2.1-20240328-ctx4096.pth) |**⚙️ 2025年2月中旬** |
...
对于大多数用例，你应该使用最新的 [RWKV-6-World-14B-V2.1](https://hf-mirror.com/BlinkDL/rwkv-6-world/blob/main/RWKV-x060-World-14B-v2.1-20240719-ctx4096.pth) 模型。如果你的硬件配置不足以使用 14B 参数模型，可以考虑使用 [RWKV-6-World-7B-V3](https://hf-mirror.com/BlinkDL/rwkv-6-world/blob/main/RWKV-x060-World-7B-v3-20241112-ctx4096.pth) 模型。

请查看[本地部署 RWKV 模型的硬件性能需求](https://rwkv.cn/RWKV-Wiki/How-To-Play#vram-of-rwkv)，以确认使用哪一款 RWKV 模型。
