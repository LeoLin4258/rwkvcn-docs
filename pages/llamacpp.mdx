...
<Tabs items={[' 下载现成 gguf 模型（推荐）', '从 HF 格式转换成 .gguf']}>
  <Tabs.Tab> 
可以从 [RWKV-6-World-GGUF 仓库](https://modelscope.cn/collections/RWKV-6-World-GGUF-f06d2c9def1349) 下载已量化并转化成 gguf 格式的 RWKV 模型。
...
<Callout type="warning" emoji="⚠️">
不添加 `-no-cnv` 参数时，`./llama-cli` 默认是对话模式。添加 `-no-cnv` 参数，设置为续写模式，此处模型会根据给定的 prompt 继续生成文本。
</Callout>
...
删除 `-no-cnv ` 参数，或设置参数为 `-cnv ` ，可以开启 llama.cpp 的命令行**对话模式**。例如：
...
参数解释：`-cnv`: 开启对话模式（conversation mode），其他参数的含义和续写模式一致。
