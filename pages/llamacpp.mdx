...
<Tabs items={[' 下载现成 gguf 模型（推荐）', '从 HF 格式转换成 .gguf']}>
  <Tabs.Tab> 
可以从 [RWKV-GGUF 仓库](https://modelscope.cn/organization/RWKV?tab=collection) 下载已量化并转化成 gguf 格式的 RWKV 模型。
...
RWKV gguf 模型有多种量化类型，精度越高模型的回复效果越好，但模型体积和计算要求越高。

推荐顺序：`FP16` > `Q8_0` > `Q5_K_M` > `Q4_K_M` ，更低的量化精度（如 `Q3_0`、`Q2_0` 等）可能会大大降低模型的性能，甚至无法正常回答问题。
</Callout>
...
```
<Callout type="warning" emoji="⚠️">
 请将上述命令中的 `llama.cpp/convert_hf_to_gguf.py` 和 `./v6-Finch-1B6-HF` 改成你下载的代码和 RWKV HF 模型目录。
</Callout>
<Callout type="info" emoji="ℹ️">
...
``` bash copy
./llama-cli -m models/rwkv7-0.4B-world-F16.gguf -p "You are a helpful assistant" -cnv -t 8 -ngl 99 -n 500
```
...
- `-m models/rwkv7-0.4B-world-F16.gguf`：模型的路径参数
...
``` bash copy
./llama-cli -m models/rwkv7-0.4B-world-F16.gguf -p "User: What's mbti?tell me in chinese.\n\nAssistant:" -no-cnv -t 8 -ngl 99 -n 500
```

![RWKV 模型续写推理](./images/llamacpp/llama.cpp-RWKV-inference-single-prompt.png)

- `-p "User: What's mbti?tell me in chinese.\n\nAssistant:"`： prompt 参数，模型根据该提示词进行续写。更多 RWKV prompt 格式请在 [RWKV 文档-提示词指南](https://rwkv.cn/RWKV-Prompts/Chat-Prompts "RWKV文档-提示词指南")中查看
...
``` bash copy
./llama-quantize models/rwkv7-0.4B-world-F16.gguf models/rwkv-6-world-1b6-Q8_0.gguf Q8_0
```
<Callout type="warning" emoji="⚠️">
