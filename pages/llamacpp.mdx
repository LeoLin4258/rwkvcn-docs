...
description: llama.cpp 支持推理 RWKV 模型，本教程指导你如何使用 llama.cpp 推理 RWKV 模型。
keywords: llama.cpp RWKV推理,RWKV模型llamacpp推理,RWKV模型llamacpp推理教程,RWKV模型量化推理
---
...
<Tabs items={[' 下载现成 gguf 模型（推荐）', '从 HF 格式转换成 .gguf']}>
  <Tabs.Tab> 
可以从 [RWKV - GGUF 仓库](https://hf-mirror.com/latestissue) 下载已量化并转化成 gguf 格式的 RWKV 模型。
...
建议下载 `Q8_0` 和 `Q5_1` 两种量化精度的 gguf 模型，更低的量化可能会使模型的回答变得很差。
</Callout>

  </Tabs.Tab>
  <Tabs.Tab> 
  首先，从 [RWKV - HF 仓库](https://huggingface.co/RWKV) 或[镜像仓库](https://hf-mirror.com/RWKV)（国内可访问）下载一个 Hugging Face 格式的 RWKV 模型，如 `RWKV/v6-Finch-1B6-HF`。
...
<Callout type="info" emoji="ℹ️">
 如果你已经下载了 `.pth` 格式的模型，可以使用附录中的 [pth-to-HF 脚本](#appendix-code) 将模型转为 HF 格式，再从 HF 格式转为 gguf 格式。
</Callout>
  </Tabs.Tab>
</Tabs>
...
``` bash copy
./llama-cli -m models/rwkv-6-world-7b-Q8_0.gguf -p "User: What's mbti?tell me in chinese.\n\nAssistant:" -t 8 -ngl 99 -n 500 --no-context-shift
```

这条命令通过 llama-cli 运行 `models/rwkv-6-world-7b-Q8_0.gguf` 模型，使用 8 个线程、跳过预热、并根据给定的 prompt 生成最多 500 个 token。

![RWKV 模型推理](./images/llamacpp/llama.cpp-RWKV-inference-single-prompt.png)

**参数解释：**

- `./llama-cli`：启动编译好的 llama-cli 程序
- `-m models/rwkv-6-world-7b-Q8_0.gguf`：模型的路径参数
- `-p "User: What's mbti?tell me in chinese.\n\nAssistant:"`： prompt 参数，模型根据该提示词生成文本。更多 RWKV prompt 格式请在[RWKV文档-提示词指南](https://rwkv.cn/RWKV-Prompts/Chat-Prompts "RWKV文档-提示词指南")中查看。
- `-t 8`：-t 指定线程数，建议根据可用的物理 CPU 核心数调整
- `- ngl`：指定模型使用的 n-gpu-layers ，可以设定 `-ngl 99`，使 llama.cpp 加载 RWKV 模型所有层
- `-n 500`：-n 参数表示生成的最大 token 数
- `--no-context-shift`：RWKV 模型不支持 Transformer 的 context shift 机制，因此需要添加 `--no-context-shift` 参数
<Callout type="info" emoji="ℹ️">
完整的参数列表可以在 [llama.cpp 参数文档](https://github.com/ggerganov/llama.cpp/blob/master/examples/main/README.md)中查看。
</Callout>
</Steps>
...
``` bash copy
./llama-server -m models/rwkv-6-world-7b-Q8_0.gguf -ngl 99 --no-context-shift
```

启动后，可以通过 `http://127.0.0.1:8080` 访问 llama.cpp Web 页面：

![WebUI](./images/llamacpp/llama.cpp-chatui-new-version.png)

{/*
### 批量推理生成（存在问题，等待 llama.cpp 修复）
...
``` bash copy
./llama-parallel -ns 4 -np 4 -m models/rwkv-6-world-7b-Q8_0.gguf -ngl 25 -n 500 -p "Who are you?\nWhat do we have for dinner?\nWhat's the meaning of life\nHello\nWhat is the end of the universe?" --no-context-shift
```
...
![可选的量化精度](./images/llamacpp/llama.cpp-quantization-type.png)



{/*
## 搭载 RWKV State 文件 （等待更新）
