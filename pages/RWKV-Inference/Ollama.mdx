...
- **从 [Ollama 的 RWKV-6 仓库](https://ollama.com/mollysama/rwkv-6-world)下载**：操作简单，但 Ollama 提供的 RWKV 模型**只有 `q4_k_m` 量化版本**
- **自定义 RWKV 模型**：需要手动下载 `gguf` 格式的 RWKV 模型并创建 `Modelfile` 配置文件，但是可以**自由运行任意量化的 RWKV 模型**

<Tabs items={['从 Ollama 仓库下载', '自定义 RWKV 模型']}>
<Tabs.Tab>
[Ollama 的 RWKV-6 仓库](https://ollama.com/mollysama/rwkv-6-world) 提供了 1.6B、3B、7B、14B 四种参数规模的 RWKV-6-World 模型。

![ollama-rwkv-6-model-repo](./imgs/ollama-rwkv-6-model-repo.png)

在你的终端执行 `ollama run mollysama/rwkv-6-world:1.6b` 命令，即可自动**下载并运行** `q4_k_m` 量化版本的 RWKV-6-World 1.6B 模型。

![ollama-run-rwkv-6-world-1.6b](./imgs/ollama-run-rwkv-6-world-1.6b.png)

<Callout type="info" emoji="ℹ️">
可以把 `1.6b` 换成 `3b`、`7b` 或 `14b`，以运行其他参数规模的 RWKV-6-World 模型。
</Callout>
</Tabs.Tab>
<Tabs.Tab>
...
要自定义 RWKV 模型，需要先从 [RWKV - GGUF 仓库](https://modelscope.cn/organization/RWKV?tab=collection) 或 [Hugging Face 镜像站](https://hf-mirror.com/latestissue)下载 `gguf` 格式的 RWKV-6-World 模型。
