...
| 0.5 | 1.3 | 适度增加随机性，避免内容太死板 |
| 0.3 | 1.5 | 采样范围有限，需要适当增加随机性 |
|  0.1 | 1.8   | 低 `Top_P` 容易生成重复内容，提高 `Temperature` 增加变化性 |
...
| 0.3      | 0.6        | `Top_P` 较低，采样范围受限，增加 `Presence Penalty` 避免相同的词或短语反复出现。 |
| ≤ 0.2    | 0.1，建议 `Temperature` 1.5 | `Top_P` 过低时**可用的词汇很少**，不应强行惩罚已出现的词，否则会造成表达异常，建议通过提高 `Temperature` 增强变化性。 |
...
| 0.3      | 0.6              | `Top_P` 低时，需更高的 `Frequency Penalty` 防止重复 |
| ≤ 0.2    | 0.1，且建议 `Temperature` 1.5 |  **`Top_P` 极低**（≤ `0.2`）时，建议减少 `Frequency Penalty`，改为提高 `Temperature`，否则可能导致文本异常、不流畅。  |
</Tab>
