...
但是 `Presence Penalty` 惩罚的强度并不是一成不变的，在 `Penalty Decay` 参数的作用下，`Presence Penalty` 惩罚会随着文本的长度增加而逐步减弱，衰减公式：$P_{\mathrm{effective}} = P_0 \cdot \gamma^d$，其中 $P_0$ 是原始惩罚值，$\gamma$ 是 `Penalty Decay` 值，$d$ 是与“美”上次出现的位置的距离。
...
下面先给出 RWKV-7 G1 模型卡的参数起点，再保留两组按任务收紧随机性的通用示例。其他架构或微调模型应以对应模型卡为准。

| 任务类型 | Top_P | Temperature | Presence Penalty | Frequency Penalty | Penalty Decay |
|---------|--------|--------------|------------------|------------------|----------------|
| G1 创意写作 | 0.6 ~ 0.8 | 0.6 | 2.0 | 0.2 | 0.99 |
| G1 普通聊天 | 0.5 | 1.0 | 2.0 | 0.1 | 0.99 |
| 保守型任务，回答稳定且准确 | 0.3 | 0.3 | 0.0 | 0.0 | 0.996 |
| G1f 1.5B+ 函数调用（模型卡参数） | 0.0 | 0.0 | 0.0 | 0.0 | 保持默认值（不生效） |
| 机械任务，无随机性 | 0.0 | 0.0 | 0.0 | 0.0 | 保持默认值（不生效） |

- **创意类内容创作**：典型的例子是小说、故事创作，需要平衡 `Temperature` 和 `Top_P`，提供更多的**随机性**和**开放性**，让模型能生成更具想象力的表达。
- **G1 普通聊天**：采用当前模型卡给出的聊天基线；其他架构或微调模型可能需要不同参数。
- **保守型任务**：如知识问答、代码任务等，需要稳定且准确的回答，因此大幅降低 `Top_P` 和 `Temperature` ，同时移除 `Presence Penalty` 和 `Frequency Penalty` 带来的惩罚。
- **函数调用**：RWKV-7 G1 模型卡为 G1f 1.5B 及更大模型给出的建议是将 `Temperature`、`Top_P`、`Presence Penalty` 和 `Frequency Penalty` 设为 0。还需配合对应的 Function Call 模板，并测试目标模型的输出稳定性。
- **机械任务**：如选择题、判断题等，需要严格遵循特定格式，只输出唯一解。可以将 `Temperature`、`Top_P`、`Presence Penalty` 和 `Frequency Penalty` 设为 0，以取消随机性。两个惩罚参数均为 0 时，`Penalty Decay` 不会影响输出，保持后端默认值即可。
