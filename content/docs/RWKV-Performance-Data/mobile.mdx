---
title: 移动端和边缘设备
description: RWKV在移动端芯片上的推理性能，包括高通、MTK Soc 以及各类嵌入式/边缘计算设备。
keywords: [RWKV移动端推理速度, rwkv高通推理速度, RWKV在MTK Soc上的推理速度, RWKV在RK3588上的推理速度]
---
...
<CallOut type="info">
RWKV 模型在移动端和边缘设备的推理性能，包括高通、MTK Soc 和 RK3588 等各类嵌入式/边缘计算设备。移动端速度会受 SoC、散热状态、系统与驱动、运行后端、量化格式和测试时长影响，只有测试条件一致时才适合直接比较。
</CallOut>
