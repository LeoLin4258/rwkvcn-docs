...
<CallOut type="info">
RWKV 在 Apple Silicon 和摩尔线程等其他硬件上的推理性能，包括 Apple Silicon、MTT-S4000 等硬件。不同运行时、量化方式、线程数和测试脚本的 TPS 不适合直接横向比较。
</CallOut>
...
## 摩尔线程 MTT-S4000（历史 RWKV-6 数据）

以下数据有多次交叉验证，但对应的是 RWKV-6 和当时的软件环境，因此仅作为历史性能参考。
