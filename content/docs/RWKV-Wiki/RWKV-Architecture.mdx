...
|**[RWKV-4-World](https://huggingface.co/BlinkDL/rwkv-4-world)**|基于 100 多种全球语言数据集的预训练语言模型|[RWKV World v1](./FAQ#怎样读懂-rwkv-模型文件名) (590B tokens)|[rwkv_vocab_v20230424](https://github.com/BlinkDL/RWKV-LM/blob/main/RWKV-v5/tokenizer/rwkv_vocab_v20230424.txt) (65,536)|
...
RWKV-5.2 架构发布了 0.4B、1B5、3B、7B，以及 3B（ctx16k）五个模型，这些模型基于 World-v2 数据集训练。
...
| RWKV-5.2 | 在 RWKV-5.1 的基础上引入了对角衰减矩阵，也就是将 $u$、$w$ 向量参数分别进行对角化                                                        | [RWKV-5-World-V2](https://huggingface.co/BlinkDL/rwkv-5-world) 系列，参数包含 0.4B、1B5、3B、7B，以及 3B（ctx16k） | World-v2                                                                                                                                   |

<CallOut type="error">
RWKV-5 全系列模型均已过时，RWKV-6 已停止更新，建议使用持续更新的 RWKV7-G1 系列模型。
</CallOut>

### RWKV-6

RWKV-6 的版本代号是“Finch（雀）”，这个版本自 2023 年 10 月开发。
...
$$
{wkv}_{t} =  \mathrm{diag}(u)\cdot k_{t}^\mathrm{T} \cdot v_{t} + \sum_{i=0}^{t-1}  \mathrm{diag}\left(\bigodot_{j=i+1}^{t-1}w_{j}\right) \cdot  k_{i}^\mathrm{T} \cdot v_{i}  \in \mathbb{R}^{(D/h) \times (D/h)}
$$
...
| RWKV-6-World-v2.1 | 已停止更新 | 基于 World-v2.1 数据集的多语言预训练模型                              | World-v2.1                 |
| RWKV-6-World-v3   | 已停止更新 | 基于 World-v3 数据集的多语言预训练模型                              | World-v3                 |
| RWKV-6-ChnNovel   | 已停止更新 | 中文小说微调模型，在 RWKV-6-World-v2.1 的基础上，使用中文小说+指令数据微调 | World-v2.1，中文小说数据 |
| RWKV-6-Jpn        | 已停止更新 | 日文微调模型，在 RWKV-6-World-v2.1 的基础上，使用日文+指令数据微调 | World-v2                 |
...
有关 RWKV State 文件的加载方法，请查看 [RWKV state 文件用法](../../tutorials/basic/State)。
</CallOut>

RWKV 官方发布了以下适用于 RWKV-6 的 **state 文件**，目前尚未发布 RWKV-7 State：

| state 类型                        | state 描述                                                     | 适用 RWKV 模型               |
|--------------------------------|----------------------------------------------------------|------------------------|
| chn-single-round               | 中文单轮对话增强，更符合人类语言习惯，带丰富的 Emoji 表情 | 同参数规模的 RWKV-6-World 模型         |
| eng-single-round               | 英文单轮对话增强，更符合人类语言习惯，带丰富的 Emoji 表情 | 同参数规模的 RWKV-6-World 模型         |
| chn-小说扩写-single-round       | 中文单轮对话，会根据用户输入进行小说扩写（建议使用 RWKV-6-ChnNovel 模型和 state 作为替代）                 | 同参数规模的 RWKV-6-World 模型       |
| chn-打油诗-single-round         | 中文单轮对话，会根据用户输入创作打油诗                   | 同参数规模的 RWKV-6-World 模型         |
| chn-文言文-single-round         | 中文单轮对话，回答的风格会偏向文言文                     | 同参数规模的 RWKV-6-World 模型         |
| chn-文言文和古典名著-single-round | 中文单轮对话，回答的风格会偏向文言文和古典名著           | 同参数规模的 RWKV-6-World 模型         |
...
<CallOut type="info" >
RWKV-7 采用了**动态状态演化**（Dynamic State Evolution）。通过一系列创新（例如广义 Delta Rule），RWKV-7 在保持线性时间和恒定空间推理特性的同时，进一步提升了状态表达能力和下游任务表现。

在训练数据远低于其他领先 3B 模型的前提下，RWKV-7 论文中的 2.9B 模型在多语言下游任务上达到当时的 SoTA 水平，并在英语下游任务上达到与当时 SoTA 相当的水平。

通过引入广义 Delta Rule，RWKV-7 使用**单层即可解决属于 $NC^1$ 完全类的 $S_5$ 状态跟踪问题**，**使用 4 层即可识别所有正则语言**；在标准复杂度猜想下，其表达力超越了 Transformers 的 $TC^0$ 限制。
</CallOut>
...
| 模型版本 | 时间步公式 | 状态更新机制 |
| --- | --- | --- |
| RWKV-5 Eagle | $S_t = S_{t-1}\mathrm{diag}(w) + v_t^\mathrm{T}k_t$ | Trainable State Decay |
| RWKV-6 Finch | $S_t = S_{t-1}\mathrm{diag}(w_t) + v_t^\mathrm{T}k_t$ | Dynamic State Decay |
| RWKV-7 Goose | $S_t = S_{t-1}\left(\mathrm{diag}(w_t) + a_t^\mathrm{T}b_t\right) + v_t^\mathrm{T}k_t$ | Dynamic State Evolution |
...
- RWKV7-G1（"GooseOne"）是持续更新的基底模型系列，预训练数据包括网页、代码、合成数据，以及指令、聊天和推理数据
...
RWKV7-G1 是持续更新的基底模型系列，拥有较强的推理、代码和数学能力。

`G1x` 中的 `x` 表示数据版本。具体可用的模型规模和检查点请以模型仓库为准。
...
<CallOut type="info">
RWKV-8 仍处于实验阶段。下文整理了已经公开的 DeepEmbed、ROSA 机制及相关实验。
</CallOut>

#### RWKV-V8 的 DeepEmbed[#rwkv-8-de]

RWKV-8 首个特性 `DeepEmbed` 在 2025 年 5 月[公布](https://rwkv.cn/news/read?id=20250527)。`DeepEmbed` 可以实现类似 MoE 的优秀推理性能，新增参数无需常驻显存，并可进一步 offload 至 RAM 或 SSD，使其更适合在端侧设备上部署。
...
<CallOut type="info" >
由于 DeepEmbed 的查表向量无需常驻 VRAM，这些向量在参数量层面几乎是"免费的"。因此，还可以进一步引入 n-gram（如 `bigram`、`trigram`），提升模型对词组/片段的建模能力。如果词表规模较大，也可结合 LoRA 技术降低显存和训练开销。
</CallOut>

RWKV7-G1 仓库中的 rwkv7a 模型使用了[**更强的 DeepEmbed 设计**](https://github.com/BlinkDL/RWKV-LM/blob/main/RWKV-v7/rwkv_v7a_demo.py#L257C1-L263C21)：
...
RWKV-8 的第二个特性 `ROSA` 于 2025 年 10 月[公开](https://zhuanlan.zhihu.com/p/1960369250049951081)。
...
- [ROSA-Tuning](https://github.com/zyaaa-ux/ROSA-Tuning) (训练 ROSA)：将 ROSA 机制与现代 LLM 相结合。该项目使模型能够仅通过固定长度的注意力窗口处理任意长度的输入，同时实现接近全局注意力（Global Attention）的性能表现。
- [ROSA+](https://github.com/bcml-ai/rosa-plus)：ROSA+ 是 ROSA 机制的扩展版本。它提供了直观的 Python 接口，并引入了 Witten–Bell 预测器作为处理未知序列时的回退（Fallback）方案。
...
对于长度为 $T$ 的序列，Transformer 注意力的计算复杂度通常为 $O(T^2)$，自回归推理所需的 KV 缓存则随上下文长度按 $O(T)$ 增长。RWKV 处理完整序列的时间复杂度为 $O(T)$，循环状态的空间复杂度为 $O(1)$，因此单个 token 的推理时间和状态大小不会随上下文长度增长。
