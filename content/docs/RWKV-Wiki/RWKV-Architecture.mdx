...
$$
\begin{align*}
a_{t + 1} &= \exp(W) \odot a_t + \exp(Kz_t) \odot (Vz_t) \qquad b_{t+1} = \exp(W) \odot b_t + \exp(Kz_t)  
\\ \\
...
RWKV-V8 的第二个特性 `ROSA` 已于 2025 年 10 月[正式发布](https://zhuanlan.zhihu.com/p/1960369250049951081)。
...
$\text{ROSA}(x)$ 是一种针对 $x$ 的高效、无参数次标记（Next-token）预测器。下图展示了其在 Pile 数据集 10,000 份长文档上的预测错误率与 Token 位置的关系。可以看到，随着上下文长度（Ctxlen）的增加，错误率持续下降。

![prediction-error](./imgs/RWKV-Architecture/prediction-error.png)

Naïve ROSA 在设计上即可实现 100% 的 MQAR（多查询关联检索）和 100% 的 NIAH（“大海捞针”测试，例如 “best thing to do in San Francisco is”），且不受上下文长度影响。但出于一些显而易见的原因，它**直接用于大多数实际任务时效果有限**。
