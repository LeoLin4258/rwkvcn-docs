...
<CallOut type="info">
RWKV-8 架构仍处于实验阶段。RWKV-8 已公开的特性：
...
- [RWKV-8 系列之 ROSA（Rapid Online Suffix Automaton） 机制：开启未来之路](./RWKV-Architecture.mdx#rwkv-8-rosa-mechanism)
</CallOut>

<Tabs items={['RWKV-7 架构', 'RWKV-6 架构', 'RWKV-4 架构']}>
  <Tab>

RWKV-7 架构是 RWKV 的最新正式版本，架构论文于 2025 年 3 月 18 日正式发布。
...
目前，RWKV-4/5 模型的生命周期已结束，RWKV-6 模型已停止更新。推荐使用**持续更新的 RWKV7-G1 模型**。
</CallOut>
...
| 总体状态 | **⚠ EOL** | **⚠ EOL**| **⚠ EOL** | **⚠ EOL** | **持续更新** |
| 0.1B 模型 | ✅ 已发布 | ✅ 已发布 | ❌ 无计划 |✅ [已发布](https://modelscope.cn/models/Blink_DL/rwkv-7-world/files) | ✅ [已发布](https://modelscope.cn/models/Blink_DL/rwkv7-g1/files) |
| 0.4B 模型 | ✅ 已发布 | ✅ 已发布 | ❌ 无计划 |✅ [已发布](https://modelscope.cn/models/Blink_DL/rwkv-7-world/files) | ✅ [已发布](https://modelscope.cn/models/Blink_DL/rwkv7-g1/files) |
...
| 3B 模型 | ✅ 已发布 | ✅ 已发布 | ✅ [已发布](https://modelscope.cn/models/Blink_DL/rwkv-6-world/files) |✅ [已发布](https://modelscope.cn/models/Blink_DL/rwkv-7-world/files) | ✅ [已发布](https://modelscope.cn/models/Blink_DL/rwkv7-g1/files) |
...
| --- | --- |
| **rwkv7** | RWKV-7 架构 |
| **g1j** | G1 系列的 Data Version（数据版本）标识。G1x 表示训练超过 1 个 epoch，后缀表示继续加入更多、更新且质量更高的数据 |
| **2.9b** | 模型的参数规模，"B"代表"Billions"（十亿） |
| **20260831** | checkpoint 日期（通常按 `YYYYMMDD` 记录） |
| **ctx16384** | 预训练的上下文长度 |
| **.pth** | PyTorch 权重文件 |
</Tab>
  <Tab>
...
由于老旧架构导致的性能问题， RWKV-6、RWKV-5、RWKV-4 全系列（Raven / World / Pile ...）和更早的 RWKV 版本均已结束生命周期，新项目不再推荐使用；复现旧实验或加载已有 State 时，仍需使用匹配的历史模型。
</CallOut>
...
请查看[本地部署 RWKV 模型的显存需求](./Vram)，了解模型规模和量化方式对资源占用的影响；实际占用请以所用模型和后端为准。

## RWKV 与 Transformer 的差异[#rwkv-vs-transformer]
...
| **RWKV-4/5/6/7** | RWKV 的不同架构版本。注意，推荐使用持续更新的 RWKV7-G1 系列模型 |
...
| **RWKV7-G1（Goose One）** | 基于 RWKV-7 架构训练的基底模型，支持**思考推理**（Think），性能更强 |
