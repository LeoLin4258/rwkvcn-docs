---
title: RWKV 的训练数据集
description: RWKV使用World系列训练数据集，包含全球一百多种语言。World系列训练数据集持续迭代，使RWKV模型拥有强劲的多语言能力。
keywords: [RWKV数据集, RWKV训练数据, RWKV数据集组成, RWKV数据集来源, RWKV数据集质量]
---
import { CallOut } from 'components-docs/call-out/call-out.tsx'

RWKV 使用 **World 系列训练数据集**，覆盖一百多种语言。本页整理论文和模型发布页已经公开的数据组成、来源与规模；未公开的训练配比不会根据模型名称猜测。

## RWKV 数据集概况

目前 World 数据集共有以下版本：

| 版本号 | 已公开的总数据量 | 已公开的对应模型 |
|-------|-----------------|------------------|
...
| World v3.7       | 尚无公开、可复现的总 token 统计 | 暂不建立模型对应关系 |
...
<CallOut type="tips">
查找某个检查点的训练信息时，请优先打开它的模型卡并核对完整文件名。当前模型名中的 G1x 是 Data Version（数据版本）标识，并不等同于 World 数据集版本；文件名也不会列出具体数据组成，不能根据字母后缀或参数量反推训练数据。
</CallOut>
