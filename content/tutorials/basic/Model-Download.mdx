...
| 你准备做什么 | 模型与格式怎么选 |
| --- | --- |
| 第一次在本地聊天 | 选择当前模型卡推荐的 RWKV7-G1 权重，并从显存或内存能够容纳的参数量开始 |
| 使用 RWKV pip 或 Albatross | 下载完整 `.pth` 权重 |
| 使用 llama.cpp、KoboldCpp 或 Ollama | 下载与目标工具兼容的 GGUF 文件 |
| 在 RWKV Chat 或手机上运行 | 从移动端模型列表选择应用明确支持的格式与量化 |
| 使用函数调用 | 选择 G1f 1.5B 或模型卡标明的更新兼容权重，并使用官方函数调用模板 |
| 继续训练或微调 | 下载完整 `.pth` 权重，并记录完整文件名、模型结构和上下文长度 |

在相同参数量中，优先选择模型卡仍在推荐、日期更晚的检查点。完整文件名比单个 `G1x` 后缀更重要：它同时记录架构变体、数据版本、参数量、日期和上下文长度。
...
RWKV 模型名称中的 G1x 是 Data Version（数据版本）标识。字母后缀表示继续加入更多、更新且质量更高的数据；选择模型时，请同时核对模型卡、完整文件名、参数量和日期。

G1f 及更新的 1.5B+ 模型还提供当前模型卡中的函数调用格式。需要工具调用时，请同时采用官方 Prompt 模板和 `temperature=0、top_p=0、penalty=0`。
</CallOut>
...
<LinkCardContainer>
  <LinkCard content="Hugging Face 主站" link="https://huggingface.co/BlinkDL/rwkv7-g1/tree/main" />
    <LinkCard content="ModelScope 仓库" link="https://modelscope.cn/models/Blink_DL/rwkv7-g1/files" />
      <LinkCard content="GGUF 模型集合" link="https://huggingface.co/collections/shoumenchougou/rwkv7-gxx-gguf" />
        <LinkCard content="移动端模型" link="https://huggingface.co/HaloWang/rwkv-weights" />
</LinkCardContainer>
...
<LinkCardContainer>
  <LinkCard content="Hugging Face 主站" link="https://huggingface.co/BlinkDL/rwkv-7-world/tree/main" />
