...
| `Penalty Decay`     | 惩罚衰减参数，用于控制 `Presence Penalty` 和 `Frequency Penalty` 的衰减速度。数值越接近 1，惩罚衰减得越慢；越小则衰减越快 |
...
| 0.3 ~ 0.5 | 1.3 | 适度增加随机性，避免内容太死板 |
|  0.1 ~ 0.2 | 1.5   | 低 `Top_P` 容易生成重复内容，提高 `Temperature` 增加变化性 |
</Tab>
...
|----------|----------------------|---------|
| 0.7      | 0.2        | 允许较多词汇多样性，适当增加 `Presence Penalty` 可防止长文本中过度重复相同主题或短语。 |
| 0.5      | 0.4        | 适中搭配，保持连贯性的同时减少重复，适用于大多数任务。 |
| 0.3      | 0.6 ~ 0.8       | `Top_P` 较低，采样范围受限，增加 `Presence Penalty` 避免相同的词或短语反复出现。 |
| ≤ 0.2    | 0.1，此外建议 `Temperature` 1.5 | `Top_P` 过低时**可用的词汇很少**，不应强行惩罚已出现的词，否则会造成表达异常，建议通过提高 `Temperature` 增强变化性。 |
...
|----------|----------------------|---------|
| 0.7      | 0.2              | `Top_P` 很高时（如 `0.7`），模型可选词较多，提高 `Frequency Penalty` 可以减少冗余词汇的重复。   |
| 0.5      | 0.4              | 适用于大多数场景，既减少重复，又不影响表达 |
| 0.3      | 0.6 ~ 1              | `Top_P` 低时，需更高的 `Frequency Penalty` 防止重复 |
| ≤ 0.2    | 0.1，此外建议 `Temperature` 1.5 |  **`Top_P` 极低**（≤ `0.2`）时，建议减少 `Frequency Penalty`，改为提高 `Temperature`，否则可能导致文本异常、不流畅。  |
</Tab>
...
| 任务类型 | `Top_P` | `Temperature` | `Presence Penalty` | `Frequency Penalty` |
|---------|--------|--------------|------------------|------------------|
| 新闻 / 公告 | 0.6 | 0.7 | 0.3 | 0.4 |
| 论文 / 研究报告 | 0.4 | 0.9 | 0.4 | 0.5 |
| 产品介绍 / 文档 | 0.4 | 1 | 0.3 | 0.4 |
</Tab>
<Tab>
...
|---------|--------|--------------|------------------|------------------|
| 事实性回答 | 0.2 | 1 | 0.1 | 0.2 |
| 摘要 / 材料提取 | 0.1 | 0.9 | 0.2 | 0.3 |
| 公式 / 代码生成 | 0.1 | 0.7 | 0.1 | 0.1 |
| 翻译 / 选择题 / 判断题 | 0 | 0 | 0 | 0 |
</Tab>

</Tabs>

以上参数仅供参考，可以根据具体任务再微调参数，调整原则：

- **`Top_P` 控制选词范围**：数值高词汇更多样，数值低词汇匮乏但更精准。  
