...
本文首先介绍面向 RWKV-7 G1 权重的 CUDA Graph 推理，再通过 `API_DEMO_CHAT.py` 解释有状态聊天循环。安装示例使用 `rwkv==0.8.32`：
...
os.environ["RWKV_V7_ON"] = '1'       # 开启 RWKV-7 模型支持
os.environ['RWKV_JIT_ON'] = '1'      # 开启 JIT (Just-In-Time) 编译，加速算子加载
os.environ["RWKV_CUDA_ON"] = '1'     # 开启 CUDA 自定义算子（需要 CUDA 编译器）
...
**模型版本：**`RWKV_V7_ON` 仅适用于 RWKV-7 模型。加载 RWKV-6 或 RWKV-5 模型时，请将其设为 `0` 或删除这一行。

**CUDA 自定义算子：**`RWKV_CUDA_ON='1'` 需要系统能够使用 `nvcc`。如果编译失败，请将它设为 `0`，改用 PyTorch 实现。
</CallOut>
...
TOP_P = 0.0             # Top-p 采样：0.0 只保留概率最高的 token
...
RWKV-7 当前仅支持 `cuda` 或 `cpu` 搭配 `fp16`、`fp32`、`bf16`。本例使用 `strategy='cuda fp16'`。如果显存不足，请先换用更小的模型。
...
os.environ["RWKV_CUDA_ON"] = "0"  # 禁用 CUDA 自定义算子
```
<CallOut type="warning">
在推理 RWKV-7 模型时，请务必将 `os.environ["RWKV_V7_ON"]` 设置为 `1` 。

如果将 `RWKV_CUDA_ON` 设为 `1`，需要安装 C++ 编译器和 CUDA Toolkit。编译失败时，请改回 `0`。
</CallOut>
...
## RWKV-4/5/6 的推理精度与显存需求[#RWKV-pip-strategy-and-vram-requirements]

<CallOut type="info">
下表适用于 RWKV-4/5/6，不适用于 RWKV-7。
...
| **cuda fp16i8 \*20 -> cuda fp16**   | VRAM 占用介于 fp16 和 fp16i8 之间 | 前 20 层使用 fp16i8，其余层使用 fp16。<br />VRAM 不足时增加 fp16i8 层数；VRAM 有余时减少 fp16i8 层数。 |
| **cuda fp16i8 \*20+**               | 比 fp16i8 使用更少 VRAM           | 前 20 层使用 fp16i8 并固定在 GPU，其他层按需加载。未固定层的加载速度约慢 3 倍，但可以节省 VRAM。<br />VRAM 不足时减少固定层数；VRAM 有余时增加固定层数。 |
| **cuda fp16i8 \*20 -> cpu fp32**    | 比 fp16i8 使用更少 VRAM，但消耗更多内存 | 前 20 层使用 fp16i8 并固定在 GPU，其他层使用 CPU fp32。CPU 性能较强时，这种方式可能比仅在 GPU 固定部分层更快。<br />VRAM 不足时减少 GPU 层数；VRAM 有余时增加 GPU 层数。 |
| **cuda:0 fp16 \*20 -> cuda:1 fp16** | 使用双卡驱动模型                  | GPU 0 加载前 20 层，GPU 1 加载其余层。<br />优先让速度更快的 GPU 运行更多层；单卡显存不足时可将 fp16 改为 fp16i8。 |
