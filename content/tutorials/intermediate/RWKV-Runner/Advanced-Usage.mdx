---
title: 进阶用法
description: 详细介绍RWKV Runner的进阶用法，包括Strategy和量化、自定义模型配置、搭载state文件、Function Call功能、服务器部署等。
keywords: [RWKV Runner进阶用法, RWKV Runner Strategy和量化, RWKV Runner自定义模型, RWKV Runner搭载state文件, RWKV Runner服务器部署]
---
...
```mermaid
graph TD
    A[RWKV Runner] -->| 来自 llama.cpp 项目 | L[llama.cpp]
    A -->| 来自 rwkv pip 包 | PIP-CPU[CPU（不推荐）]
    A -->| 来自 rwkv.cpp  项目 | B[CPU-RWKV.cpp]
    A -->| 来自 rwkv pip 包 | PIP-CUDA[CUDA]
    A -->| 来自 web-rwkv-py 项目 | C[webgpu-python]

        L -->|推理设备| GPU[VULKANGPU]
        L -->|推理设备| CPU[CPU]
