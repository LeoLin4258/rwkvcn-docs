---
title: RWKV-LM 预训练教程
description: 基于 RWKV-LM-V7 的 RWKV-7 预训练方法
...
本教程基于社区维护的 [RWKV-LM-V7](https://github.com/RWKV-Vibe/RWKV-LM-V7)，适合快速复现约 191M～3B 的 RWKV-7 预训练。该仓库仍处于 WIP，官方参考实现请查看 [RWKV-LM 的 `RWKV-v7/train_temp`](https://github.com/BlinkDL/RWKV-LM/tree/main/RWKV-v7/train_temp)。
...
在相同模型规模和上下文长度下，预训练通常比 LoRA、MiSS 等参数高效微调需要更多显存。

以下是基于 `HEAD_CHUNK=4096`，`GRAD_CP=1`，`KERNEL="@rwkv3"` 参数，在不同 `CTX_LEN` 下预训练 RWKV 模型所需的显存：

<Tabs items={["CTX_LEN=8192", "CTX_LEN=4096"]}>
<Tab>

| 模型参数 | 层数 | 维度 | M_BSZ=1 显存 | 其他 M_BSZ 显存 |
| -------- | ---- | ---- | --------- | ------------- |
| 0.1B     | 12   | 768  | 6786MB（6.63GB） | BSZ8：11412MB（11.14GB）<br />BSZ48：40556MB（39.61GB） × 4 |
| 0.4B     | 24   | 1024 | 12938MB（12.63GB） | BSZ6：18828MB（18.39GB）<br />BSZ8：20366MB（19.89GB） |
...
安装完成后，使用以下命令检查 PyTorch 版本以及 PyTorch 能否使用 CUDA：

```bash
python -c "import torch; print(torch.__version__, torch.cuda.is_available())"
```
若输出类似 `2.7.1+cu128 True`，表示当前 PyTorch 已经识别 CUDA GPU。这个结果不能用于判断系统 CUDA Toolkit 或 `nvcc` 是否可用。
...
| 参数                               | 参数解释                                                                                                       |
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| `MODEL_TYPE="x070"`                | 训练的**模型版本**，当前 RWKV-LM-V7 使用 RWKV-7 的 `x070` 架构                                                    |
| `N_LAYER="12"` `N_EMBD="768"`      | **模型层数和维度**，维度和层数决定预训练模型的参数大小，建议参考[硬件需求](#hardware-requirements)修改，N_EMBD 必须 64 的倍数 |
| `CTX_LEN="4096"`                   | 预训练的**上下文长度**，必须是 16 的倍数，并与准备训练数据时使用的 `--ctx_len` 参数相同                                 |
...
| `MODEL_TYPE="x070"`                | 训练的**模型版本**，当前 RWKV-LM-V7 使用 RWKV-7 的 `x070` 架构                                                    |
| `N_LAYER="12"` `N_EMBD="768"`      | **模型层数和维度**，维度和层数决定预训练模型的参数大小，建议参考[硬件需求](#hardware-requirements)修改，N_EMBD 必须 64 的倍数 |
| `CTX_LEN="4096"`                   | 预训练的**上下文长度**，必须是 16 的倍数，并与初始化模型、准备训练数据时使用的值一致                                  |
| `data_file`                        | 预训练的**数据路径**，使用已转换的 bin 和 idx 数据，无需文件名后缀                                                 |
| `--my_exit_tokens`                 | 训练数据集的**总 token 数**，会在全部 token 训练完毕后退出，在 `make_data.py` 中计算得到                                    |
| `--magic_prime`                    | 训练数据集的**magic_prime 值**，在 `make_data.py` 中计算得到                                                        |
...
| `M_BSZ=16`                     | 建议为 2 的幂；显存允许时可以适当增大，以提高训练吞吐                                                                                                                                   |
...
| `GRAD_CP=1`                    | 是否启用梯度检查点；`GRAD_CP=1` 节省显存但训练较慢，`GRAD_CP=0` 训练更快但消耗更多显存                                                                                                         |
...
| `DS_BUCKET_MB=2`               | DeepSpeed bucket size（单位 MB）。当前示例脚本已禁用该参数；如需启用，请先根据固定的 DeepSpeed 版本验证兼容性                                                                                         |
| `--load_model "0"`             | 保持默认值 `0`。训练中断后，继续训练会自动检测最新检查点                                                                                                                       |
| `--wandb "Test"`               | **建议注册一个 WandB 账号并填写你的项目名称，以便观察和比较 loss。详情查看附录 [使用 WandB 监控训练过程](#wandb)**                                                             |
...
| `--data_type binidx`           | 当前训练脚本仅支持 `binidx` 数据                                                                                               |
| `--vocab_size 65536`           | 词表大小，必须与训练数据使用的 tokenizer 一致；本教程使用 RWKV World 词表，保持 `65536`                                                                                 |
| `--weight_decay 0.001`         | 权重衰减，保持默认值 `0.001`                                                                                                                                                   |
| `--head_size 64`               | 头大小，保持默认值 `64`                                                                                                                                                        |
| `--accelerator gpu`            | 加速器类型，必须是 `gpu`                                                                                                                                                       |
| `--precision bf16`             | 训练精度，当前 RWKV-7 CUDA 算子使用 `bf16`，请保持该值                                                                                                                                 |
| `--strategy deepspeed_stage_2` | 训练策略，默认 `deepspeed_stage_2`，更多训练策略请参考[DeepSpeed 训练策略](#deepspeed)                                                                              |
...
<CallOut type="warning">
`demo-training-run.sh` 开头包含删除部分检查点的 `rm` 命令。如果 `PROJ_DIR` 中已有需要保留的检查点，请先注释这些命令并核对目标目录。
</CallOut>
...
<CallOut type="warning">
继续预训练 RWKV7-G1 模型时，请在 [`src/model.py`](https://github.com/RWKV-Vibe/RWKV-LM-V7/blob/main/src/model.py) 中核对 `D_DECAY_LORA`、`D_AAA_LORA`、`D_MV_LORA` 和 `D_GATE_LORA`，确保这些维度与基底模型参数完全对齐。
</CallOut>
...
本教程使用的 PyTorch Lightning 提供以下 DeepSpeed 策略名称：

| 策略名称 | 分片内容 | 主要取舍 |
|---|---|---|
| deepspeed_stage_1 | 优化器状态 | 通信开销相对较低，GPU 显存节省有限 |
| deepspeed_stage_2 | 优化器状态和梯度 | 进一步节省 GPU 显存，同时增加通信开销 |
| deepspeed_stage_2_offload | Stage 2，并将优化器状态卸载到 CPU | 继续降低 GPU 显存占用，速度取决于 CPU 与传输带宽 |
| deepspeed_stage_3 | 优化器状态、梯度和模型参数 | GPU 显存节省最多，但通信与调度更复杂 |
| deepspeed_stage_3_offload | Stage 3，并将优化器状态和模型参数卸载到 CPU | 进一步降低 GPU 显存占用，训练速度更依赖传输带宽 |

本页示例使用 `deepspeed_stage_2`。实际训练时应根据模型规模、GPU 数量、显存和互联带宽选择策略。
...
<Accordions type="single">
  <Accordion title="训练中断了，如何继续训练？">
    `demo-training-run.sh` 会从 `PROJ_DIR` 中查找最新检查点，但脚本开头也包含删除部分检查点的 `rm` 命令。继续训练前，请先注释这些 `rm` 命令，再运行 `sh demo-training-run.sh`，并在终端中确认 `### Loading ...` 指向预期的检查点。
  </Accordion>
  <Accordion title="训练速度太慢，如何加快训练？">
    - 在显存允许的情况下，尽可能增大 `M_BSZ` 参数，增大 `M_BSZ` 会增大显存占用，但也会加快训练速度
    - 根据模型规模、GPU 数量和互联带宽选择合适的 DeepSpeed 策略
    - 显存允许时可关闭梯度检查点：`GRAD_CP=0` 通常训练更快，但会消耗更多显存
  </Accordion>
