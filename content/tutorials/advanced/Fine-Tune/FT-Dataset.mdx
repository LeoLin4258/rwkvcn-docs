...
复制训练数据不会增加数据多样性，只会提高这些样本在训练中的出现频率。只有在确实需要提高某类样本权重时，才建议复制；随机排序只会改变样本顺序，不能替代补充高质量、多样化的数据。

在 GNU/Linux 或 WSL 环境中，使用以下命令对数据文件进行重复和打乱：
...
<CallOut type="info">
Windows 系统可以通过 [WSL](https://docs.microsoft.com/zh-cn/windows/wsl/install) (Windows Subsystem for Linux) 或安装 [Cygwin](https://www.cygwin.com/) 工具，以使用 `awk` 和 `sort` 命令。
</CallOut>
...
RWKV-PEFT 可以直接读取 `jsonl` 数据，也支持 `binidx`。如果训练脚本设置为 `--data_type binidx`，需要使用 [json2binidx](https://github.com/Abel2076/json2binidx_tool) 工具，将 `jsonl` 文件转换成 `binidx` 文件。
...
得到 `json2binidx_tool` 文件夹后，使用 `cd json2binidx_tool` 命令进入 `json2binidx_tool` 目录。
</Step>
<Step>
### jsonl 文件转 binidx 文件

将准备好的 `jsonl` 数据集文件放在 `json2binidx_tool/data` 文件夹中。
...
以上述参数为例，工具会生成 `./data/sample_text_document.bin` 和 `./data/sample_text_document.idx`。在训练配置中，`data_file` 应填写 `./data/sample_text_document`，不需要添加扩展名。
</Step>
</Steps>
