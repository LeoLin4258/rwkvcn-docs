---
title: 微调环境配置
description: 为你的 Linux 系统配置 RWKV 微调环境，包括安装 CUDA Toolkit、配置 Conda 虚拟环境、安装训练所需的软件、检查 CUDA 环境等。
...
本教程使用 Conda 管理 Python 环境。此处以 Miniconda 为例，介绍如何配置 Linux 系统的 Conda 环境。
...
## 检查 CUDA 环境[#check-cuda]

上述依赖安装完毕后，可以检查当前 PyTorch 是否能够识别 CUDA 和 NVIDIA GPU。
...
如果安装了 CUDA 版本的 PyTorch，仍然返回 `CUDA available: False`，请先使用 `nvidia-smi` 检查显卡驱动和 GPU 状态，并确认当前终端已经激活正确的 Conda 环境。如果需要重新安装 CUDA Toolkit，请参阅 [**CUDA Toolkit 安装文档**](#cudatoolkit)。
