---
title: MiSS 微调教程
description: MiSS 微调是一种参数高效微调方法，通过分片共享结构减少可训练参数，并在模型效果与资源使用之间提供另一种取舍。
...
本文的 MiSS 微调方法来自 RWKV 社区微调项目 [RWKV-PEFT](https://github.com/Joluck/RWKV-PEFT)。
...
MiSS 支持直接读取 JSONL 数据，也可以使用转换后的 binidx 数据，具体格式和准备方法可参考[准备微调数据集](../FT-Dataset)。
...
- data_file：训练数据集的路径。使用 `jsonl` 时需要填写包含 `.jsonl` 后缀的文件路径；使用 `binidx` 时填写数据文件前缀，不带 `.bin` 和 `.idx` 后缀。
</Step>
<Step>
...
# --lr_schedule wsd （使用 WSD 学习率调度，不加此参数则默认使用 cos）
```
