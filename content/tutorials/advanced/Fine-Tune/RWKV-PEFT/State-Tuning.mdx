...
State Tuning 只训练 RWKV 模型的初始 State，不修改基底模型权重。训练结果是一个需要与原基底模型配套加载的 State 文件。
</CallOut>
本文的 State tuning 方法来自 RWKV 社区微调项目 [RWKV-PEFT](https://github.com/Joluck/RWKV-PEFT)。

开始之前，请确保你拥有一个 Linux 工作区，以及支持 CUDA 的 NVIDIA 显卡。

## State tuning 微调案例[#state-tuning-case]

以下是社区训练的 RWKV-7 State 案例。目前 RWKV 官方尚未发布 RWKV-7 State，使用社区 State 时，请同时确认其对应的基底模型。
...
<div className="iframe-container">
 <iframe 
 src="https://player.bilibili.com/player.html?isOutside=true&aid=115744855557576&bvid=BV18sqdBPE3e&cid=34837695336&high_quality=1&autoplay=0"
 scrolling="no"
 frameBorder="0"
...
- data_file：训练数据集路径。使用 `jsonl` 时填写包含 `.jsonl` 后缀的完整文件路径；使用 `binidx` 时填写不带 `.bin` 和 `.idx` 后缀的文件名前缀。
</Step>
<Step>
...
当前上游 State Tuning 示例使用 `--lr_init 1e-5` 和 `--lr_final 1e-5`。建议先按示例完成一次训练，再根据训练结果调整学习率和 `ctx_len`。
</CallOut>

| 参数 | 描述 |
| --- | --- |
| `--data_type jsonl` | 训练语料的文件格式。本教程使用 `jsonl`；选择 `jsonl` 时，`data_file` 包含文件后缀；改用 `binidx` 时，`data_file` 不包含 `.bin` 和 `.idx` 后缀 |
| `--vocab_size 65536` | 词表大小，必须与基底模型一致；RWKV World 模型使用 `65536` |
| `--epoch_count 5` | 总训练轮次，可根据效果调整 |
| `--pre_ffn 0` | 用 ffn 替换第一个 att 层，有时可能有益 |
| `--lr_init 1e-5` | 初始学习率，当前上游 State Tuning 示例使用 `1e-5` |
| `--lr_final 1e-5` | 最终学习率，当前上游 State Tuning 示例使用 `1e-5` |
| `--accelerator gpu` | 使用的加速器类型，目前主要支持 `gpu`， `cpu` 基本不支持训练 |
| `--devices 1` | 单显卡填 `1`，多卡按实际数量填写 |
| `--precision bf16` | 训练精度，当前上游 State Tuning 脚本使用 `bf16`；支持：`fp32`、`tf32`、`fp16`、`bf16` |
| `--strategy deepspeed_stage_1` | lightning 训练策略参数，微调推荐使用 `deepspeed_stage_1` |
| `--grad_cp 1` | 梯度检查点开关；`1` 可以节省显存，但会降低训练速度 |
| `--peft state` | 微调训练类型，state tuning 微调填 `state` 即可 |
| `--my_testing "x070"` | 训练的 RWKV 模型版本，v7 选 `x070`，v6 选 `x060` ，v5 选 `x052`（已淘汰，不推荐）|
| `--op fla` | State Tuning 使用的算子；与 `--peft state` 配合设置为 `fla` |
| `--wandb PEFT-State-tuning` | **可选**，是否使用 wandb 可视化记录训练日志，需提前配置 [wandb](https://wandb.ai/) 账号 |
| `--lr_schedule cos` | **可选**，学习率调度方法，默认为 `cos`，支持：`cos`、`wsd` |
...
挂载 state 文件的方法可以参考 [RWKV Runner](../../../intermediate/RWKV-Runner/Advanced-Usage#挂载-state) |  [Ai00](../../../intermediate/ai00/Ai00-Features)
</CallOut>
<CallOut type="warning" >
