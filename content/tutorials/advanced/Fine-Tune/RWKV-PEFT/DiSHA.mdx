---
title: DiSHA 微调教程
description: DiSHA微调是一种新颖的参数高效微调方法，可进一步降低训练参数同时加速收敛，DiSHA在评估指标、计算效率和资源使用方面均优于LoRA及其变体。
keywords: [RWKV DiSHA 微调, DiSHA 微调教程, DiSHA 微调方法, DiSHA 微调框架, DiSHA 微调效果]
---
...
| `epoch_save=5` | 每隔多少个训练轮次保存一次 DiSHA 文件，注意存储空间是否充足 |
