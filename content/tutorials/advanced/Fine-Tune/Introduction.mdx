---
title: 微调简介
description: 为什么要微调 RWKV 模型，以及需要为微调训练准备什么。
keywords: [微调 RWKV 模型, RWKV微调, 微调 RWKV 模型需要什么, 微调 RWKV 环境, 微调 RWKV 模型需要什么显卡]
---
...
目前官方发布的 RWKV-7 G1x 系列模型均为基底模型（base model ，又称预训练模型），基底模型在自然语言处理等领域的大规模数据集上进行了训练，具备较强的泛化能力和丰富的知识储备。

但为了保持泛化能力和通用性，RWKV 基底模型并未针对某一类任务作优化。因此，RWKV 模型在某些特定任务上的表现可能不够理想。

而对 RWKV 模型进行微调，通俗地说，是**使用特定领域（如法律、文学、医学等）或任务（材料总结、小说续写等）的高质量数据集对 RWKV 模型进行再次训练**。如果数据和训练配置合适，微调可以改善 RWKV 模型在对应任务上的表现。

相比于从头训练一个全新的模型，微调是在预训练模型的基础上继续训练，通常需要的训练周期和计算资源更少。
...
<Tabs items={["State tuning", "LoRA 微调", "MiSS 微调"]}>
<Tab>
